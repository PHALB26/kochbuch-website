/* ===========================
   Veganes Jahreszeiten-Kochbuch
   - Portions-Skalierung (Seite) + Inline
   - Kochmodus (Utensilien → Zutaten → Schritte)
   - Eigene Portionsleiste NUR im Kochmodus
   - Schritt-Timer NUR im Kochmodus
   - Wake Lock NUR im Kochmodus
   - Schriftgrößen-Regler NUR im Kochmodus (immer 100% Start)
   =========================== */
...
function pluralize(sing, plur, amt){
  if(!sing) return '';
  if(!plur) return sing;
  const n = Number(amt);
...
const originalPortions = getOriginalPortions();
...
function scaleInlinePage(root=document){
  const factor = currentPortions / originalPortions;
...
function mirrorSteps(){ const z = $('#zubereitung'); const m=$('#steps'); if(z&&m){ m.innerHTML = z.innerHTML; } }
function scaleAllPage(){ scaleIngredientsPage(); scaleInlinePage($('#zubereitung')); mirrorSteps(); }

function changePortion(delta){ currentPortions = Math.max(1, currentPortions+delta); updatePortionText(); scaleAllPage(); }
...
function copyIngredients(){
  ...
  navigator.clipboard.writeText(text).then(()=>alert('Zutaten wurden kopiert.')).catch(()=>{
...
const cookState = {
  portions: null,
  basePortions: originalPortions
};

/* Scaling nur innerhalb eines Roots (Kochmodus) */
function cook_scaleIngredientsIn(root){
  const factor = cookState.portions / cookState.basePortions;
...
function injectCookFSControls(){
  ...
  // vor den Fortschrittskreis einfügen
  const anchor = $('#step-progress');
  if (anchor) {
    modalInner.insertBefore(box, anchor);
  } else {
    modalInner.appendChild(box);
  }
  applyCookFontSize(pct);
}
...
function renderCookStep(){
  ...
  if(item.type==='list'){
    ...
    const p=document.createElement('p'); p.style.fontSize='1.05rem'; p.style.lineHeight='1.6';
    p.appendChild(item.node); content.appendChild(p);
...
function startCookingMode(){
  ...
  cookState.basePortions = originalPortions;
  cookState.portions = currentPortions || originalPortions;
...
(function(){
  ...
  function alarm(){
    ...
    if(el){ el.loop=true; return { start(){ try{ el.currentTime=0; el.play().catch(()=>{});}catch{} }, stop(){ try{ el.pause(); el.currentTime=0; }catch{} } }; }
  ...
  function attach(panel,total){
    ...
    theReset=panel.querySelector('.timer-reset');
    const bReset=theReset;
    ...
    function updateSound(){ if(isSound()){ bSound.textContent='🔊'; bSound.title='Ton aus'; bSound.setAttribute('aria-label','Ton ausschalten'); } else { bSound.textContent='🔇'; bSound.title='Ton an'; bSound.setAttribute('aria-label','Ton einschalten'); } }
...
document.addEventListener('DOMContentLoaded', ()=>{
  // Seiten-Portion initialisieren
  const pt = $('#portion-text'); if(pt){ const m=pt.textContent.match(/\d+/); currentPortions = m?parseInt(m[0],10):originalPortions; }
  updatePortionText(); scaleAllPage();
